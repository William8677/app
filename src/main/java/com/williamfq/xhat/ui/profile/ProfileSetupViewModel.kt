...
class ProfileSetupViewModel @Inject constructor(
    ...
    private fun loadInitialData() {
        viewModelScope.launch {
            try {
                val savedProfile = userRepository.getCurrentProfile()
    ...
    // Se corrige la conversión del timestamp a una fecha formateada (dd/MM/yyyy)
    ...
    // Método agregado para resetear el estado de navegación tras un éxito
    fun resetUiState() {
    ...
    private suspend fun validateUsername(username: String): String? {
        return when {
            ...
            userRepository.isUsernameTaken(username) -> "Nombre de usuario no disponible"
            else -> null
        }
    }
    ...
    fun saveProfile() {
        viewModelScope.launch {
            try {
                ...
                val currentProfile = userRepository.getCurrentProfile()
